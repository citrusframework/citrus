...
public class JsonPatchFunction implements ParameterizedFunction<JsonPatchFunction.Parameters> {
    ...
    private String convertToPatchOpJson(PatchOperation op) {
        String jsonPointer = convertToJsonPointer(op.path());

        if ("remove".equals(op.operation())) {
            return String.format("{\"op\":\"%s\",\"path\":\"%s\"}", op.operation(), jsonPointer);
        } else if ("move".equals(op.operation()) || "copy".equals(op.operation())) {
            String fromPointer = convertToJsonPointer(op.value());
            return String.format("{\"op\":\"%s\",\"path\":\"%s\",\"from\":\"%s\"}",
                op.operation(), jsonPointer, fromPointer);
        } else {
            String jsonValue;
            try {
                objectMapper.readTree(op.value());
                jsonValue = op.value();
            } catch (Exception e) {
                jsonValue = "\"" + escapeJsonString(op.value()) + "\"";
            }
            return String.format("{\"op\":\"%s\",\"path\":\"%s\",\"value\":%s}",
                op.operation(), jsonPointer, jsonValue);
        }
    }
    ...
    public record PatchOperation(String operation, String path, String value) {}
