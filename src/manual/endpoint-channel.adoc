...
Message selectors are described in link:#message-selectors[message-selector].
...
in link:#message-selectors[message-selector].
...
channel. The XPath expression outcome must match an expected value and only then the message is consumed form the channel.
...
channel. The JsonPath expression outcome must match an expected value and only then the message is consumed form the channel.
