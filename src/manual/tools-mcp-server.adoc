...
* Settings resources
    ** **Settings Catalog**: Browse all available Citrus system property and environment variable settings by group or module
...
[[citrus-mcp-server-install-maven]]
=== Maven Dependency
...
[[citrus-mcp-server-ai-tools]]
=== Integrate with AI assistance tools

[[citrus-mcp-server-install-plugin]]
==== Install as a plugin

Claude Code can install the Citrus MCP server from the marketplace defined in the `citrusframework/citrus`
repository, so you do not have to edit any configuration file by hand. JBang must still be installed
and available on your PATH.

[source,bash]
----
claude plugin marketplace add citrusframework/citrus
claude plugin install citrus-mcp@citrus-marketplace
----

The first command registers the marketplace; the second installs the `citrus-mcp` plugin, which ships
the MCP server configuration. The server is launched on demand over STDIO via JBang.

[[citrus-mcp-server-install-manual]]
==== Manual configuration

Alternatively, you can integrate the Citrus MCP server into AI code assistance tools using an MCP configuration file.
The exact file name and location depends on the AI tool you are using (see <<citrus-mcp-server-config-locations>> below).
...
TIP: JBang supports the `LATEST` version placeholder.
Use `org.citrusframework:citrus-mcp-server:LATEST:runner` to always fetch the latest available release from Maven Central without having to specify a concrete version number.

NOTE: You can also run the server directly without using JBang - then it would be something like `java -jar <FULL PATH>/citrus-mcp-server-${citrus.version}-runner.jar`.
JBang might be the simpler approach though because it automatically deals with downloading the JAR file from Maven central.
...
[[citrus-mcp-server-config-locations]]
==== Configuration file locations

Each AI tool has its own conventions for where the MCP configuration file lives.
The table below shows both project and global locations for each tool.
When adding the configuration to the project root directory the configuration applies only to sessions started in that project.
Global configuration applies to all sessions for that AI tool (`~/` refers to your individual user home directory).
...
| IBM Bob
| `.bob/mcp.json` in project root directory
| `~/.bob/settings/mcp.json` or `~/.bob/settings/mcp_settings.json` (Bob 1.x)

| VS Code / GitHub Copilot
| `.vscode/mcp.json` in project root directory
| VS Code user `settings.json` under the `"mcp"` key

| JetBrains IDEs (2025.1+)
| `.junie/mcp.json` in project root directory or in `Settings > Tools > AI Assistant > MCP Servers`
| `~/.junie/mcp.json`
|===

IMPORTANT: VS Code uses `"servers"` as the top-level key instead of `"mcpServers"`.
...
The settings resources provide a catalog of all Citrus system property and environment variable settings.
Each setting entry includes the system property key, environment variable name, default value, value type, and a description.

==== All Settings

**URI:** `citrus://settings/all`

Retrieves all available Citrus settings grouped by category.
...
----
{
  "core": {
    "name": "core",
    "title": "Core Settings",
    "description": "Core Citrus framework settings from CitrusSettings",
    "module": "citrus-api",
    "settings": [
      {
        "property": "citrus.default.message.type",
        "envVariable": "CITRUS_DEFAULT_MESSAGE_TYPE",
        "defaultValue": "XML",
        "type": "string",
        "description": "Default message type for validation"
      }
    ]
  },
  "logging": { ... },
...
**URI Template:** `citrus://settings/group/{name}`

Retrieves settings for a specific group (e.g., core, logging, camel, kubernetes, testcontainers).
...
**Example URI:** `citrus://settings/group/kubernetes`

==== Settings by Module

**URI Template:** `citrus://settings/module/{module}`

Retrieves settings for a specific Citrus module (e.g., citrus-api, citrus-camel, citrus-kubernetes, citrus-testcontainers).
A module may contain multiple setting groups.
For example, `citrus-camel` contains the groups `camel`, `camel-cli`, and `camel-infra`.

**Parameters:**

* `module`: Citrus module name

**MIME Type:** `application/json`

**Example URI:** `citrus://settings/module/citrus-camel`
