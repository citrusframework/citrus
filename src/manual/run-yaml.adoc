...
As usual, you need to choose one of the supported test engines in Citrus to actually run the test (e.g. link:#runtime-junit-jupiter[JUnit Jupiter] or link:#runtime-testng[TestNG]).
...
public class MyFirstYamlIT extends TestNGCitrusSpringSupport {
    ...
    @Test
    @CitrusTestSource(type = "yaml", name="my.citrus.it.yaml")
