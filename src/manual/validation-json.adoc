...
The Json parser mode is also configurable via system property `citrus.json.permissive.mode` or environment variable `CITRUS_JSON_PERMISSIVE_MODE`.
Accepted values are: `SIMPLE` (default), `RFC4627`, `STRICTEST`, `PERMISSIVE`, or a numeric parser mode value.
This controls how strictly the underlying JSON parser handles the input format.
...
[source,xml,indent=0,role="secondary"]
----
<bean id="defaultGroovyJsonMessageValidator" class="org.citrusframework.validation.groovy.GroovyJsonMessageValidator"/>
----
...
receive(someEndpoint)
    .message()
    .type(MessageType.JSON)
    .validate(validation().jsonPath().expressions(validationMap));
----
