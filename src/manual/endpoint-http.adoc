...
TIP: For requests that must carry a Playwright browser session (the cookies and HTTP credentials of
a logged-in browser), use an Http client whose transport is the browser. See <<playwright-api-requests>>.
...
The Http server provides the following global settings configurable via system properties or environment variables:

.System properties
|===
|System property |Description

| citrus.http.server.response.cache.size
| Number of response messages the server keeps in its in-memory cache for message tracing (default="100")

| citrus.http.server.use.default.filters
| Enable/disable default servlet filters (e.g. request caching filter) on the server's servlet context (default="true")
|===
...
| CITRUS_HTTP_SERVER_RESPONSE_CACHE_SIZE
| Number of response messages the server keeps in its in-memory cache for message tracing (default="100")

| CITRUS_HTTP_SERVER_USE_DEFAULT_FILTERS
| Enable/disable default servlet filters (e.g. request caching filter) on the server's servlet context (default="true")
|===
...
<bean id="citrusHttpMessageController" class="org.citrusframework.http.controller.HttpMessageController">
  <property name="endpointAdapter">
      <bean
       class="org.citrusframework.base.endpoint.adapter.EmptyResponseEndpointAdapter"/>
  </property>
</bean>
----
