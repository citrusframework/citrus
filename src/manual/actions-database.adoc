...
*Important*
Do not mix statements of type *_SELECT_* with others in a single sql test action. This will lead to errors because validation steps are not valid
...
*Important*
The validation with *_"<validate column="..." value="..."/>"_* meets single row result sets as you specify a single column control value. In case you have multiple rows in a result set you rather need to validate the columns with multiple control values like this:
