...
The Kafka message broker is composed of a Kafka server that uses some default configuration options. Citrus provides an embedded server (*for testing purpose only!*)
that is able to start within your integration test environment. The server cluster is configured with one single Kafka node. You can define server ports and broker properties such as topics, number of partitions and
...
start and stop within the application context lifecycle. The server log directory is located in the Java temp directory
...
| log-dir-path
| java.lang.String
| Path to server log directory. The server will create its data directory in this directory. By default, the
