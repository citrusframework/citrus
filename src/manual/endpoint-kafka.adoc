...
| message-converter
| No
| `org.citrusframework.citrus.kafka.message.KafkaMessageConverter`
...
| header-mapper
| No
| `org.citrusframework.citrus.kafka.message.KafkaMessageHeaderMapper`
| Header mapper maps Kafka record information (e.g. topic name, timestamp, message key) to internal message headers
  (`org.citrusframework.citrus.kafka.message.KafkaMessageHeaders`) and vice versa.
