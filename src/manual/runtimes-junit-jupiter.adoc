...
Citrus provides extensions in order to enable Citrus related dependency injection and parameter resolving in your JUnit test.

NOTE: The JUnit support is shipped in a separate Maven module. You need to include the module as a dependency in your
project.

.JUnit module dependency
[source,xml]
----
<dependency>
...
[[junit-jupiter-citrus-support]]
=== Citrus extension

You can use the Citrus JUnit extension on your test as follows:
...
The class above is using the JUnit `@Test` annotation as a normal unit test would do. The `@CitrusSupport` annotation marks
the test to use the Citrus JUnit extension. This enables us to use the `@CitrusTest` annotation on the test and adds
...
to enable the Citrus support for JUnit.
...
[[junit-jupiter-endpoint-injection]]
=== Endpoint injection
...
.JUnit Citrus endpoint injection
[source,java]
----
package org.citrusframework.samples;
...
[[junit-jupiter-citrus-spring-support]]
=== Citrus Spring extension

Spring is a famous dependency injection framework that also provides support for JUnit. Citrus is able to load its
components as Spring beans in an application context. The Citrus JUnit extension works great with the Spring extension.

The Spring extension loads the application context and Citrus adds all components to the Spring bean configuration.

.JUnit Citrus Spring test
[source, java]
----
@CitrusSpringSupport
...
[[junit-jupiter-test-sharding]]
=== Test Sharding

You can run your JUnit test cases in order in a distributed manner, facilitating test sharding.
