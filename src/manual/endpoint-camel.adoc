...
== Camel route statistics reset

The reset route stats action resets Camel route statistics using the `camel-management` dependency (`ManagedCamelContext`).
It performs a deep reset via `ManagedRouteMBean.reset(true)`, which clears all counters including processor-level statistics.
This is useful when you need a clean baseline before verifying route statistics in tests that share a Camel context.
...
[source,java,indent=0,role="primary"]
----
public class CamelResetRouteStatsIT extends TestNGCitrusSpringSupport {
...
[source,xml,indent=0,role="secondary"]
----
<testcase name="CamelResetRouteStatsIT">
  <actions>
      <camel:reset-route-stats route="route_1"/>
  </actions>
</testcase>
----
...
You can combine the reset action with the verify action to ensure a clean baseline before making assertions:
...
<camel:reset-route-stats route="route_1"/>
<!-- ... trigger some exchanges ... -->
<camel:verify-route-stats route="route_1" completed="1" failed="0"/>
----
...
The Apache Camel project offers a great CLI tooling that allows you to run Camel integrations directly from the command line.
Citrus supports two ways to launch the Camel CLI:

* *JBang* (default) — uses https://www.jbang.dev/[JBang] to resolve and run the Camel CLI application.
* *Camel Launcher jar* — uses a standalone Camel Launcher jar file executed via `java -jar`.

[[camel-cli-jbang-setup]]
=== Camel JBang setup

By default, Citrus uses JBang to launch the Camel CLI.
You can install the Camel CLI tooling on your local machine as a JBang application.
...
The `--help` option will show you all available commands that you can run with Camel CLI.

[[camel-cli-launcher-setup]]
=== Camel Launcher setup

As an alternative to JBang you can use the Camel Launcher jar to run Camel CLI commands.
The Camel Launcher is a standalone executable jar that does not require JBang to be installed.
...
citrus.camel.cli.launcher.jar.path=/path/to/camel-launcher.jar
----

.Environment variables
[source,bash]
----
export CITRUS_CAMEL_CLI_TYPE=launcher
export CITRUS_CAMEL_CLI_LAUNCHER_JAR_PATH=/path/to/camel-launcher.jar
----

When configured, Citrus will execute Camel CLI commands as `java -jar <path-to-jar> <command> <args>` instead of using JBang.
...
The Citrus test is able to run this route with Camel CLI in a separate process:
...
[source,xml,indent=0,role="secondary"]
----
<test name="CamelCliTest" xmlns="http://citrusframework.org/schema/xml/testcase"
      xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
...
This starts the Camel integration with Camel CLI.
...
You can also use the Camel CLI send command without a running Camel integration.
...
TIP: You can also use the Kamelets sources to send messages with Camel CLI. The Kamelets provide easy to use properties for comfortable connection options.

Please see the Camel CLI send command help for more configuration options.

.Camel CLI cmd send options
[source,shell]
----
camel cmd send --help
----
...
The Camel CLI tooling allows you to access stubbed messages in a running Camel integration.
...
Camel CLI supports plugins that extend the CLI with additional commands and capabilities.
...
TIP: The test action has an `autoRemove` option that is disabled by default. When enabled, the test will automatically delete the plugin after the test using the `context.doFinally()` mechanism. You can enable it globally via the `citrus.camel.cli.auto.remove.plugins` system property or `CITRUS_CAMEL_CLI_AUTO_REMOVE_PLUGINS` environment variable.
...
The Camel CLI support uses the following environment settings.
...
| The launcher type to use: `jbang` (default) or `launcher`. When set to `launcher`, Citrus uses the Camel Launcher jar instead of JBang.

| citrus.camel.cli.launcher.jar.path
| Path to the Camel Launcher jar file. Required when `citrus.camel.cli.type` is set to `launcher`. Falls back to the `camel.launcher.jar` system property.
...
| The launcher type to use: `jbang` (default) or `launcher`.
...
[[camel-cli-kubernetes]]
== Camel CLI Kubernetes support
