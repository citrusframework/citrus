...
Citrus creates HTML reports after each test run. The report provides detailed information on the test run with a summary of all test results. You can find the report after a test run in the directory *_${project.build.directory}/test-output/citrus-reports_* .
...
citrus.html.report.directory:: Output directory path (default= *_${project.build.directory}/test-output/citrus-reports_*).
