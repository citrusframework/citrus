...
public class KafkaContainerResource extends TestcontainersResource<ConfluentKafkaContainer>
        implements QuarkusTestResourceConfigurableLifecycleManager<KafkaContainerSupport> {

    public KafkaContainerResource() {
        super(ConfluentKafkaContainer.class);
    }

    @Override
    public void init(KafkaContainerSupport config) {
        for (Class<? extends ContainerLifecycleListener<ConfluentKafkaContainer>> lifecycleListenerType :
    ...
    protected void doInit(Map<String, String> initArgs) {
        StartKafkaAction.Builder<ConfluentKafkaContainer> builder = new StartKafkaAction.Builder<ConfluentKafkaContainer>()
                .implementation(KafkaImplementation.CONFLUENT);
