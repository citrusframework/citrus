 ...
 *     <li>Min value: The minimum value for the generated random number (optional, default: {@link #DEFAULT_MIN_VALUE}).</li>
 *     <li>Max value: The maximum value for the generated random number (optional, default: {@link #DEFAULT_MAX_VALUE}).</li>
...
public class AdvancedRandomNumberFunction implements Function {
    ...
    public String execute(List<String> parameterList, TestContext context) {
        ...
        String formatPattern =  getParameter(parameterList, 6, String.class, v->v, null);

        BigDecimal randomValue =  getRandomNumber(decimalPlaces, minValue, maxValue, excludeMin, excludeMax, multiple);
     ...
     */
    private BigDecimal getRandomNumber(int decimalPlaces,
                                   BigDecimal minValue,
                                   BigDecimal maxValue,
                                   boolean excludeMin,
                                   boolean excludeMax,
    ...
    public static String formatRandomNumber(BigDecimal randomValue, int decimalPlaces, String formatPattern) {
        ...
        if (formatPattern != null && !formatPattern.isEmpty()) {
            DecimalFormatSymbols symbols = new DecimalFormatSymbols(Locale.US); // Ensure '.' as decimal separator
        ...
        return decimalPlaces == 0 ?
            randomValue.toPlainString() :
            randomValue.setScale(decimalPlaces, RoundingMode.HALF_UP).toPlainString();
    }

    private static boolean isWithinDoubleRange(BigDecimal value) {
        BigDecimal maxDouble = BigDecimal.valueOf(Double.MAX_VALUE);
        BigDecimal minDouble = maxDouble.negate(); // -Double.MAX_VALUE
