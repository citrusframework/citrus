...
public class ZooTestRunnerTest extends UnitTestSupport {
    ...
    public void testZookeeperBuilder() throws KeeperException, InterruptedException {
        ...
        MockTestRunner builder = new MockTestRunner(getClass().getSimpleName(), context) {
            @Override
            public void execute() {
                zookeeper(builder -> builder.client(new org.citrusframework.citrus.zookeeper.client.ZooClient(zookeeperClientMock))
        ...
        Assert.assertEquals(action.getCommand().getClass(), org.citrusframework.citrus.zookeeper.command.Info.class);

        action = (ZooExecuteAction) test.getActions().get(1);
        Assert.assertEquals(action.getName(), actionName);
        Assert.assertEquals(action.getCommand().getClass(), org.citrusframework.citrus.zookeeper.command.Create.class);

        action = (ZooExecuteAction) test.getActions().get(2);
        Assert.assertEquals(action.getName(), actionName);
        Assert.assertEquals(action.getCommand().getClass(), org.citrusframework.citrus.zookeeper.command.Delete.class);

        action = (ZooExecuteAction) test.getActions().get(3);
        Assert.assertEquals(action.getName(), actionName);
        Assert.assertEquals(action.getCommand().getClass(), org.citrusframework.citrus.zookeeper.command.Exists.class);

        action = (ZooExecuteAction) test.getActions().get(4);
        Assert.assertEquals(action.getName(), actionName);
        Assert.assertEquals(action.getCommand().getClass(), org.citrusframework.citrus.zookeeper.command.GetChildren.class);

        action = (ZooExecuteAction) test.getActions().get(5);
        Assert.assertEquals(action.getName(), actionName);
        Assert.assertEquals(action.getCommand().getClass(), org.citrusframework.citrus.zookeeper.command.GetData.class);

        action = (ZooExecuteAction) test.getActions().get(6);
        Assert.assertEquals(action.getName(), actionName);
        Assert.assertEquals(action.getCommand().getClass(), org.citrusframework.citrus.zookeeper.command.SetData.class);
    }
