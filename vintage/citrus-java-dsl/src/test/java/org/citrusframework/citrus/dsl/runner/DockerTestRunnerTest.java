...
public class DockerTestRunnerTest extends UnitTestSupport {
    ...
    public void testDockerBuilder() {
        ...
        final DockerClient client = new org.citrusframework.citrus.docker.client.DockerClient();
        ...
        Assert.assertEquals(action.getCommand().getClass(), org.citrusframework.citrus.docker.command.Info.class);

        action = (DockerExecuteAction)test.getActions().get(1);
        Assert.assertEquals(action.getName(), "docker-execute");
        Assert.assertEquals(action.getCommand().getClass(), org.citrusframework.citrus.docker.command.Ping.class);

        action = (DockerExecuteAction)test.getActions().get(2);
        Assert.assertEquals(action.getName(), "docker-execute");
        Assert.assertEquals(action.getCommand().getClass(), org.citrusframework.citrus.docker.command.Version.class);
        Assert.assertNotNull(action.getCommand().getResultCallback());

        action = (DockerExecuteAction)test.getActions().get(3);
        Assert.assertEquals(action.getName(), "docker-execute");
        Assert.assertEquals(action.getCommand().getClass(), org.citrusframework.citrus.docker.command.ContainerCreate.class);

        action = (DockerExecuteAction)test.getActions().get(4);
        Assert.assertEquals(action.getName(), "docker-execute");
        Assert.assertEquals(action.getCommand().getClass(), org.citrusframework.citrus.docker.command.ContainerInspect.class);
