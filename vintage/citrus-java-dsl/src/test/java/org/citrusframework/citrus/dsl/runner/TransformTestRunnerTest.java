...
public class TransformTestRunnerTest extends UnitTestSupport {
    ...
    public void testTransformBuilderWithResource() throws IOException {
        MockTestRunner builder = new MockTestRunner(getClass().getSimpleName(), context) {
            @Override
            public void execute() {
                transform(builder -> builder.source(new ClassPathResource("org/citrusframework/citrus/dsl/runner/transform-source.xml"))
                        .xslt(new ClassPathResource("org/citrusframework/citrus/dsl/runner/transform.xslt"))
                        .result("result"));
            }
        };
