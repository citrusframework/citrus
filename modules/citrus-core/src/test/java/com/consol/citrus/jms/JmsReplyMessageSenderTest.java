...
public class JmsReplyMessageSenderTest {
    ...
    public void testSendMessageWithJmsTemplate() {
        ...
        ((JmsSyncConsumer)sender.getJmsEndpoint().createConsumer()).saveReplyDestination(MessageBuilder.withPayload("").setHeader(JmsHeaders.REPLY_TO, replyDestination).build());
        sender.send(message);
    ...
    public void testSendMessageWithReplyMessageCorrelator() throws JMSException {
        ...
        ((JmsSyncConsumer)sender.getJmsEndpoint().createConsumer()).saveReplyDestination(requestMessage);
        sender.send(message);
