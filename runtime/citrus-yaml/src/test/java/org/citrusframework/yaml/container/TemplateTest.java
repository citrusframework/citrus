...
public class TemplateTest extends AbstractYamlActionTest {
    ...
    public void shouldLoadTemplate() {
        Template template = new TemplateLoader("classpath:org/citrusframework/yaml/container/template-test.yaml")
        ...
        template = new TemplateLoader("classpath:org/citrusframework/yaml/container/template-parameters-test.yaml")
                .withReferenceResolver(context.getReferenceResolver())
                .load()
                .build();
        Assert.assertEquals(template.getTemplateName(), "myTemplate");
