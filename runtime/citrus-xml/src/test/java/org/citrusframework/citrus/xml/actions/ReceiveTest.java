...
import org.citrusframework.citrus.message.DelegatingPathExpressionProcessor;
...
import org.citrusframework.citrus.xml.NoopMessageProcessor;
import org.citrusframework.citrus.xml.NoopVariableExtractor;
import org.citrusframework.citrus.xml.XmlTestLoader;
import org.mockito.Mockito;
import org.testng.Assert;
import org.testng.annotations.Test;

import static org.citrusframework.citrus.endpoint.direct.DirectEndpoints.direct;

/**
 * @author Christoph Deppisch
 */
public class ReceiveTest extends AbstractXmlActionTest {
    ...
    public void shouldLoadReceive() throws IOException {
        XmlTestLoader testLoader = createTestLoader("classpath:org/citrusframework/citrus/xml/actions/receive-test.xml");
        ...
        helloQueue.send(new DefaultMessage("<TestMessage>Hello Citrus</TestMessage>").setHeader("operation", "sayHello"));
        helloQueue.send(new DefaultMessage("<TestMessage>Hello Citrus</TestMessage>").setHeader("operation", "sayHello"));
        helloQueue.send(new DefaultMessage("<ns:TestMessage xmlns:ns=\"http://www.consol.com\">Hello Citrus</ns:TestMessage>").setHeader("operation", "sayHello"));
        helloQueue.send(new DefaultMessage("<TestMessage>Hello Citrus</TestMessage>").setHeader("operation", "sayHello"));
        helloQueue.send(new DefaultMessage("<TestMessage>Hello Citrus</TestMessage>").setHeader("operation", "sayHello"));
        helloQueue.send(new DefaultMessage("<TestMessage>Hello Citrus</TestMessage>").setHeader("operation", "sayHello"));
        helloQueue.send(new DefaultMessage("<TestMessage>Hello Citrus</TestMessage>").setHeader("operation", "sayHello"));
        ...
        Assert.assertEquals(messageBuilder.buildMessagePayload(context, action.getMessageType()),
                FileUtils.readToString(FileUtils.getFileResource("classpath:org/citrusframework/citrus/xml/test-request-payload.xml")));
        ...
        Assert.assertEquals(scriptValidationContext.getScriptType(), "groovy");
        Assert.assertEquals(scriptValidationContext.getValidationScriptResourcePath(), "classpath:org/citrusframework/citrus/xml/test-validation-script.groovy");
