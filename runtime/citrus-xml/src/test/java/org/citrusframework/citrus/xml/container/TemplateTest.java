...
import org.citrusframework.citrus.xml.TemplateLoader;
...
public class TemplateTest extends AbstractXmlActionTest {
    ...
    public void shouldLoadTemplate() {
        Template template = new TemplateLoader("classpath:org/citrusframework/citrus/xml/container/template-test.xml")
        ...
        template = new TemplateLoader("classpath:org/citrusframework/citrus/xml/container/template-parameters-test.xml")
                .withReferenceResolver(context.getReferenceResolver())
                .load()
                .build();
        Assert.assertEquals(template.getTemplateName(), "myTemplate");
