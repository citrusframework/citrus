...
public class CitrusBaseExtension implements BeforeAllCallback,
    ...
    protected static TestLoader createTestLoader(ExtensionContext extensionContext) {
        ...
        return new XmlTestLoader(extensionContext.getRequiredTestClass(), testName, packageName, getCitrus(extensionContext).getCitrusContext());
    }
    ...
    protected static TestContext getTestContext(ExtensionContext extensionContext) {
        Assert.notNull(extensionContext, "ExtensionContext must not be null");
        return extensionContext.getRoot().getStore(NAMESPACE).getOrComputeIfAbsent(getBaseKey(extensionContext) + TestContext.class.getSimpleName(), key -> getCitrus(extensionContext).getCitrusContext().createTestContext(), TestContext.class);
    }
