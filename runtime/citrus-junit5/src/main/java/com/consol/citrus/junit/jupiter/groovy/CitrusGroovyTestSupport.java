...
package com.consol.citrus.junit.jupiter.groovy;
 ...
 */
public final class CitrusGroovyTestSupport {

    /**
     * Prevent instantiation of utility class.
     */
    private CitrusGroovyTestSupport() {
    }
            ...
            try {
                for (String fileNamePattern : CitrusSettings.getGroovyTestFileNamePattern()) {
                    ...
                    for (Resource fileResource : fileResources) {
                        ...
                        tests.add(DynamicTest.dynamicTest(testName, testLoader::load));
                    }
                }
            } catch (IOException e) {
    ...
    public static Stream<DynamicTest> dynamicTests(String packageName, String ... testNames) {
        return Stream.of(testNames).map(testName -> {
            ...
            return DynamicTest.dynamicTest(testName, testLoader::load);
        });
    }
    ...
    private static TestLoader createTestLoader(String testName, String packageName) {
        TestLoader testLoader = TestLoader.lookup("groovy")
                .orElseThrow(() -> new CitrusRuntimeException("Missing Groovy test loader in project classpath - " +
                        "please add citrus-groovy module to the project"));
