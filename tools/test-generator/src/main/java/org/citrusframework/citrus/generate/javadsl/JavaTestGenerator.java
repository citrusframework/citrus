...
import org.citrusframework.citrus.generate.AbstractTestGenerator;
...
public class JavaTestGenerator<T extends JavaTestGenerator<T>> extends AbstractTestGenerator<T> {
    ...
    protected TypeName getBaseType() {
        if (getFramework().equals(UnitFramework.TESTNG)) {
            return ClassName.get("org.citrusframework.citrus.testng", "TestNGCitrusSupport");
        } else if (getFramework().equals(UnitFramework.JUNIT4)) {
            return ClassName.get("org.citrusframework.citrus.junit", "JUnit4CitrusSupport");
        }
    ...
    protected AnnotationSpec getBaseExtension() {
        ClassName extension = ClassName.get("org.citrusframework.citrus.junit.jupiter", "CitrusBaseExtension");
