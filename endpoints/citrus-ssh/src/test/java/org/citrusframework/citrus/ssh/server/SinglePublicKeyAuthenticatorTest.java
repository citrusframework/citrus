...
package org.citrusframework.citrus.ssh.server;
...
public class SinglePublicKeyAuthenticatorTest {
    ...
    public void withClassPath() throws IOException {
        SinglePublicKeyAuthenticator auth = new SinglePublicKeyAuthenticator("roland","classpath:org/citrusframework/citrus/ssh/allowed_test_key.pem");
        PublicKey pKey = getPublicKey("/org/citrusframework/citrus/ssh/allowed_test_key.pem");
        assertTrue(auth.authenticate("roland", pKey, null));
        assertFalse(auth.authenticate("guenther", pKey, null));

        pKey = getPublicKey("/org/citrusframework/citrus/ssh/forbidden_test_key.pem");
        assertFalse(auth.authenticate("roland", pKey, null));

        pKey = getPublicKey("/org/citrusframework/citrus/ssh/citrus.pem");
        assertFalse(auth.authenticate("citrus", pKey, null));
    }

    @Test
    public void withFile() throws IOException {
        File temp = copyToTempFile("/org/citrusframework/citrus/ssh/allowed_test_key.pem");
        ...
        temp = copyToTempFile("/org/citrusframework/citrus/ssh/forbidden_test_key.pem");
        pKey = getPublicKeyFromStream(new FileInputStream(temp));
        assertFalse(auth.authenticate("roland", pKey, null));
    }

    @Test(expectedExceptions = CitrusRuntimeException.class, expectedExceptionsMessageRegExp = ".*org/citrusframework/citrus/ssh/private.key.*")
    public void invalidKeyFormat() {
        new SinglePublicKeyAuthenticator("roland", "classpath:org/citrusframework/citrus/ssh/private.key");
    }
    ...
    public void notInClasspath() {
        new SinglePublicKeyAuthenticator("roland", "classpath:org/citrusframework/citrus/ssh/blubber.bla");
    }
