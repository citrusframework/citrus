    ...
    public void fromHeaders(MessageHeaders headers, Message jmsMessage) {
        ...
        for (Map.Entry<String, Object> headerEntry : headers.entrySet()) {
            if (filterInternalHeaders) {
                if (!headerEntry.getKey().startsWith(org.citrusframework.citrus.message.MessageHeaders.PREFIX)) {
                    integrationHeaders.put(headerEntry.getKey(), headerEntry.getValue());
                }
            } else {
                if (!headerEntry.getKey().equals(org.citrusframework.citrus.message.MessageHeaders.ID)
                        && !headerEntry.getKey().equals(org.citrusframework.citrus.message.MessageHeaders.TIMESTAMP)) {
                    integrationHeaders.put(headerEntry.getKey(), headerEntry.getValue());
                }
            }
        }
