...
public class ZooClientParser implements BeanDefinitionParser {
    ...
    public BeanDefinition parse(Element element, ParserContext parserContext) {
        ...
        if (element.hasAttribute("url")) {
            config.withUrl(element.getAttribute("url"));
        }

        if (element.hasAttribute("timeout")) {
            config.withTimeout(Integer.parseInt(element.getAttribute("timeout")));
        }

        builder.addPropertyValue("zookeeperClientConfig", config.build());
